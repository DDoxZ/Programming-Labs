...
public class Highway {
	 ...
	 * a decorrer)
	 * 
	 * @return numero total de veiculos nas filas da via rapida
	 */
     ...
     * 
     * @return numero de filas ativas nesta via rapida
	 */
     ...
     * 
     * @return numero medio de veiculos por fila ativa neste instante
	 */
     ...
     * 
     * @return tempo medio de espera de veiculos na fila
	 */
	 ...
	 * Valor total de portagens cobradas
	 * 
	 * @return valor total de portagens cobradas
	 */
     ...
     * 
     * @return valor medio de portagens cobradas por veiculo processado
     */
	...
	public void addVehicle(Vehicle v) throws IllegalQueueRequest {
		// Caso 1: ha uma fila ativa no qual podemos colocar carro
		if (q.focusMin() < maxVehiclesPerQueue) {
			q.enqueue(v);
		}
		
		// Caso 2: ha filas nao ativas que possam ser ativadas para colocar o carro
		else if (q.howManyQueues() > nrActiveQueues()) {
			boolean found = false;
			for (int i = 0; i < q.howManyQueues() && !found; i++) {
				if (!q.isActivated(i)) {
					q.activate(i);
					q.focus(i);
					q.enqueue(v);
		...
		// Caso 3: nao ha filas ativas nem nao ativas que possam ser ativadas 
		// e por isso criamos uma nova fila para colocar o carro
		else {
			q.create();
	...
	public void updateNumberActiveQueues() throws IllegalQueueRequest {
		if (nrActiveQueues() > minActivatedQueues) {
			int emptyActiveQueues = 0;
